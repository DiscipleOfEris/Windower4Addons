...
windower.register_event('addon command', function(command, ...)
  ...
  if not command or command == 'help' then
    ...
    log('e.g. //sta Maruru /ma "Cure III" <stpc>')
  ...
  elseif command == '!mirror' then
    ...
    if mirroring then
      log('Mirroring enabled. Will have all alts mimic this character.')
    else
      log('Mirroring disabled.')
    end
  elseif command == '!packets' then
    ...
    if send_packets then
      log('Packet injection enabled. This is necessary unless GearSwap is active with a profile loaded.')
    else
      log('Packet injection disabled. Do this for compatibility with GearSwap when you have an active profile.')
    end
...
windower.register_event('outgoing text', function(orig, modified, blocked, typed, a, b)
  ...
  elseif t_arg:sub(1,1) == '<' then
    ...
    local cmd = command_queue[#command_queue]
    command_queue = T{}
  ...
  if selecting and command == last_st_command and tonumber(t_arg) then
    ...
    elseif selecting == 2 then
      ...
      selecting = 3
      st_capture = modified
      if dont_send_self(command_queue[1].char) then
        if blocked then 
  ...
  -- Someone probably used //sendtarget when they should've used //send
  local cmd = command_queue:remove(#command_queue)
  return send_message(cmd.char, modified) and '' or false
end)
...
function dont_send_self(character)
  local player = windower.ffxi.get_player()
  
  if not player then return true end
  
  if character == player.name:lower() or character == '@all' or character == '@everyone' then return false end
